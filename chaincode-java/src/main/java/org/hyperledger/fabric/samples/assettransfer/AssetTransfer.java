...
public final class AssetTransfer implements ContractInterface {
    ...
    /**
     * Duplicating a asset in ledger with a different owner.
     *
     * @param ctx the transaction context
     * @param assetID the ID of the asset duplicated
     * @param newOwner the new owner
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String newOwner) {
        ...
        // Check wheather original assets exists
        if (assetJSON == null || assetJSON.isEmpty()) {
            String errorMessage = String.format("Asset %s does not exist", assetID);
            System.out.println(errorMessage);
            throw new ChaincodeException(errorMessage, AssetTransferErrors.ASSET_NOT_FOUND.toString());
        ...
        // Calculate new id of the duplicate asset by adding 1 to the length of the total assets
        int id = 0;
        ...
        //Use a Genson to conver the Asset into string, sort it alphabetically and serialize it into a json string
        String sortedJson = genson.serialize(dupAsset);
